...
<%--
    공지사항 목록 (boardList.jsp)
    1. 필요한 sql, db, dto, text, util 패키지 임포트
    2. db 연결하기
    3. sql 실행하여 result(공지사항 목록)을 가져오기
    4. 가져온 목록을 boardList(공지사항 목록)에 하나씩 담기
    5. boardList(공지사항 목록)을 테이블 태그의 tr요소를 반복하여 출력
       제목 부분 a요소에 링크 중 bno 파라미터(쿼리스티링)으로 상세보기를 요청
    6. 로그인한 사용자만 제목 부분 a요소의 링크 중 bno 파라미터(쿼리스티링)으로
        상세보기를 요청 가능
    7. 공지사항이므로 관리자만 가능하게
--%>
